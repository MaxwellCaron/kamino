...
func (f *fakeClaimsDB) Exec(_ context.Context, sql string, args ...any) (pgconn.CommandTag, error) {
	switch {
...
func (r fakeRow) Scan(dest ...any) error {
	...
	*(dest[2].(*uuid.UUID)) = r.row.actorPrincipalID
	// dest[3] is claimed_at (pgtype.Timestamptz); leaving it zero-valued is
	// fine for these tests since Claims never reads ClaimedAt.
