...
func (h *RequestsHandler) Approve(c *gin.Context) {
	...
	row, events, err := h.Service.ApproveRequest(c.Request.Context(), principalID, requestID)
	if err != nil {
		writeRequestServiceError(c, err, "approve request")
		return
	}
...
func (h *RequestsHandler) Deny(c *gin.Context) {
	...
	row, events, err := h.Service.DenyRequest(c.Request.Context(), principalID, requestID)
	if err != nil {
		writeRequestServiceError(c, err, "deny request")
		return
	}
