...
const listVisibleClonedPodVMsForPrincipal = `-- name: ListVisibleClonedPodVMsForPrincipal :many
SELECT
    cpv.cloned_pod_id,
    cpv.inventory_item_id,
    ii.name,
...
type ListVisibleClonedPodVMsForPrincipalParams struct {
	PrincipalID uuid.UUID `json:"principal_id"`
	ClonedPodID uuid.UUID `json:"cloned_pod_id"`
}

type ListVisibleClonedPodVMsForPrincipalRow struct {
	ClonedPodID     uuid.UUID `json:"cloned_pod_id"`
	InventoryItemID uuid.UUID `json:"inventory_item_id"`
	Name            string    `json:"name"`
	Node            *string   `json:"node"`
...
func (q *Queries) ListVisibleClonedPodVMsForPrincipal(ctx context.Context, arg ListVisibleClonedPodVMsForPrincipalParams) ([]ListVisibleClonedPodVMsForPrincipalRow, error) {
	rows, err := q.db.Query(ctx, listVisibleClonedPodVMsForPrincipal, arg.PrincipalID, arg.ClonedPodID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListVisibleClonedPodVMsForPrincipalRow
	for rows.Next() {
		var i ListVisibleClonedPodVMsForPrincipalRow
		if err := rows.Scan(
			&i.ClonedPodID,
			&i.InventoryItemID,
			&i.Name,
