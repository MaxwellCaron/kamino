...
const listPodVMAddressMetadata = `-- name: ListPodVMAddressMetadata :many
WITH address_metadata AS (
    ...
    WHERE assignment.inventory_item_id = ANY($1::UUID[])
    ...
    WHERE cloned_vm.inventory_item_id = ANY($1::UUID[])
    ...
    WHERE allocation.kind = 'personal_pod'
      AND allocation.inventory_item_id = ANY($1::UUID[])
)
SELECT
...
type ListPodVMAddressMetadataRow struct {
	InventoryItemID   uuid.UUID `json:"inventory_item_id"`
	NetworkNumber     int32     `json:"network_number"`
	NetworkProfileKey *string   `json:"network_profile_key"`
	LanVnet           string    `json:"lan_vnet"`
...
func (q *Queries) ListPodVMAddressMetadata(ctx context.Context, inventoryItemIds []uuid.UUID) ([]ListPodVMAddressMetadataRow, error) {
	rows, err := q.db.Query(ctx, listPodVMAddressMetadata, inventoryItemIds)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListPodVMAddressMetadataRow
	for rows.Next() {
		var i ListPodVMAddressMetadataRow
		if err := rows.Scan(
			&i.InventoryItemID,
			&i.NetworkNumber,
			&i.NetworkProfileKey,
