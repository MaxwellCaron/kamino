...
func (q *Queries) ListRequestHistory(ctx context.Context) ([]ListRequestHistoryRow, error) {
	rows, err := q.db.Query(ctx, listRequestHistory)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListRequestHistoryRow
	for rows.Next() {
