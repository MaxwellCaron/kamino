...
type CutoutCardImageBaseProps = Omit<
  UnpicImageProps,
  "layout" | "width" | "height" | "aspectRatio"
>

type CutoutCardImageDimensionProps =
  | {
      width: number
      height: number
      aspectRatio?: never
...
type FullWidthCutoutCardImageProps = CutoutCardImageBaseProps & {
  layout?: "fullWidth"
  width?: never
  height?: number
  aspectRatio?: number
...
type ConstrainedCutoutCardImageProps = CutoutCardImageBaseProps & {
  layout?: "constrained"
} & CutoutCardImageDimensionProps

type FixedCutoutCardImageProps = CutoutCardImageBaseProps & {
  layout: "fixed"
} & CutoutCardImageDimensionProps

export type CutoutCardImageProps =
  | FullWidthCutoutCardImageProps
  | ConstrainedCutoutCardImageProps
  | FixedCutoutCardImageProps

/** Defaults to Unpic `fullWidth`; parent `CutoutCardMedia` should define the rendered block size for cover images. */
export function CutoutCardImage({
  ...
  const imageClassName = cn(
    "object-cover object-top transition-transform duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] group-hover/cutout:scale-105",
    layout === "fullWidth" && "h-full w-full",
    className
  )
  ...
  if (layout === "fullWidth") {
    return (
      <Image
        ...
        {...(props as FullWidthCutoutCardImageProps)}
        layout="fullWidth"
  ...
  if (layout === "fixed") {
    return (
      <Image
        ...
        className={imageClassName}
        data-slot="cutout-card-image"
        {...(props as FixedCutoutCardImageProps)}
        layout="fixed"
        sizes={sizes}
      />
    )
  ...
  return (
    <Image
      ...
      className={imageClassName}
      data-slot="cutout-card-image"
      {...(props as ConstrainedCutoutCardImageProps)}
      layout="constrained"
      sizes={sizes}
    />
  )
