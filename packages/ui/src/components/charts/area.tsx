...
export function Area({
  ...
  const approximateLengthAtX = useCallback(
    (targetX: number) => {
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        const dCur = data[i]
        const x0 = xScale(xAccessor(dPrev))
        const x1 = xScale(xAccessor(dCur))
  ...
  const segmentBounds = useMemo(() => {
    ...
    const startX = xScale(xAccessor(startPoint))
    const endX = xScale(xAccessor(endPoint))
