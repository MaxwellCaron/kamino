...
function bisectTime(points: Array<LiveLinePoint>, time: number): number {
  let lo = 0
...
function extractLiveLineConfigs(children: ReactNode): Array<LineConfig> {
...
function LiveLineChartInner({
  ...
  const contextData = useMemo(() => {
    ...
    let startIdx = bisectTime(data, windowStart / 1000)
    if (startIdx > 0) {
      startIdx--
    }
    const sliced = data.slice(startIdx)
    const records: Array<Record<string, unknown>> = sliced.map((p) => ({
