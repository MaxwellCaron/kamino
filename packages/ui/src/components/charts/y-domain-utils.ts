...
export type YDomainRecord = Partial<Record<string, YDomain>>
...
export function computeYDomainsByAxis({
  ...
  resolveDomain: (dataKeys: Array<string>) => YDomain
}): Record<string, YDomain> {
  const groups = groupLinesByYAxisId(lines)
  const domains: YDomainRecord = {}
  ...
  return domains as Record<string, YDomain>
}
