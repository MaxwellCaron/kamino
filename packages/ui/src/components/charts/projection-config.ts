...
function normalizeProjectionData(
  data: Array<ProjectionPoint> | undefined
...
export function extractProjectionLineConfigs(
  children: ReactNode
): Array<ProjectionLineConfig> {
  const configs: Array<ProjectionLineConfig> = [];
...
export function mergeProjectionYDomain(
  ...
  configs: Array<ProjectionLineConfig>,
  yAxisId: string
): [number, number] {
  const paths = configs
