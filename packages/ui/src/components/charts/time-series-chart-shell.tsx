...
export interface TimeSeriesChartInnerProps {
  ...
  composedBarDataKeys?: Array<string>;
...
const TimeSeriesChartCore = memo(function TimeSeriesChartCoreView({
  width,
  height,
  data,
  xDataKey,
  ...
  const resolveYDomain = useCallback(
    (sourceData: Array<Record<string, unknown>>, dataKeys: Array<string>) => {
  ...
  const defsChildren: Array<ReactElement> = [];
  const clipExcludedChildren: Array<ReactElement> = [];
  const underlayChildren: Array<ReactElement> = [];
