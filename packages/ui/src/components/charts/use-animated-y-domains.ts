...
import type { YDomain, YDomainRecord } from "./y-domain-utils"
...
function snapDomains(
  ...
  animatedRef: { current: YDomainRecord }
) {
...
function tweenDomains({
  ...
  for (const axisId of axisIds) {
    ...
    const from = fromSnapshot[axisId] ?? to
  ...
  const fromByAxis: YDomainRecord = {}
  for (const axisId of axisIds) {
  ...
  const control = animate(0, 1, {
    ...
    onUpdate: (progress) => {
      ...
      for (const axisId of axisIds) {
        const to = destination[axisId]
        const from = fromByAxis[axisId] ?? to
