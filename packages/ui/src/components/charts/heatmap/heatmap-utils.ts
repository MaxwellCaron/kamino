...
export interface HeatmapSeparatorLayout {
  spacing: number
  atColumns: Array<number>
  groups: Array<HeatmapSeparatorGroup>
}
...
export function buildHeatmapSeparatorGradientStops(
  ...
  strokeOpacity = 1
): Array<HeatmapSeparatorGradientStop> {
...
export function buildHeatmapQuarterSeparatorGroups(
  ...
  const groups: Array<HeatmapSeparatorGroup> = [
    {
      startColumnIndex: 0,
...
export function buildHeatmapRowOpacity(
  match: ReadonlyArray<number> | ((row: number) => boolean),
  fadedOpacity = 0.35,
  activeOpacity = 1,
  rowCount = 7
): Array<number> {
