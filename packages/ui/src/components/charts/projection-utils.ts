...
function intervalFromAdjacentRows(
  ...
  const prev = readDate(prevRow, xDataKey);
  const current = readDate(currentRow, xDataKey);
  if (!(prev && current)) {
    return null;
  }
...
function intervalFromSeriesSpan(
  ...
  const first = readDate(firstRow, xDataKey);
  const last = readDate(lastRow, xDataKey);
  if (!(first && last)) {
    return null;
  }
...
function linearRegressionSlope(points: Array<{ t: number; y: number }>): number {
  if (points.length < 2) {
    return 0;
  }
...
function buildAutoFutureValues(options: {
  ...
  anchorValue: number;
  autoMethod: ProjectionAutoMethod;
  historyPoints: Array<{ t: number; y: number }>;
  horizonPoints: number;
  intervalMs: number;
  pathDensity: ProjectionPathDensity;
...
export function computeProjectionAnchorTangentSlope(
  ...
  const historyPoints: Array<{ t: number; y: number }> = [];
