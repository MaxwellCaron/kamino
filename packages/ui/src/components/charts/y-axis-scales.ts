...
): Map<string, Array<LineConfig>> {
  const groups = new Map<string, Array<LineConfig>>()
...
type YScaleRecord = Partial<Record<string, YScale>>

export function getPrimaryYScale(
  yScales: YScaleRecord,
  fallback: YScale
): YScale {
  ...
  const first = Object.values(yScales).find(
    (scale): scale is YScale => scale != null
  )
...
export function buildYScalesFromDomains({
  ...
  lines: Array<LineConfig>
  innerHeight: number
  domainsByAxis: Partial<Record<string, [number, number]>>
}): YScaleRecord {
  const groups = groupLinesByYAxisId(lines)
  const scales: YScaleRecord = {}
