...
export function LiveLine({
  ...
  const getX = useCallback(
    (d: Record<string, unknown>) => xScale(xAccessor(d)),
    [xScale, xAccessor]
  )
  ...
  const getY = useCallback(
    (d: Record<string, unknown>) => {
      const v = d[dataKey]
      return typeof v === "number" ? yScale(v) : 0
    },
    [dataKey, yScale]
  )
