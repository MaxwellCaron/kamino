...
function ActionBarGroup(props: DivProps) {
  ...
  const { orientation } = useActionBarContext(GROUP_NAME)
...
function ActionBarItem(props: ActionBarItemProps) {
  ...
  const { onOpenChange, orientation, loop } = useActionBarContext(ITEM_NAME)
    ...
    props: mergeProps<"button">(
      {
        ...
        className: cn(
          "ml-1 cursor-pointer rounded-xs opacity-70 outline-none hover:opacity-100 focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/50 disabled:pointer-events-none [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-3.5",
          className
        ),
